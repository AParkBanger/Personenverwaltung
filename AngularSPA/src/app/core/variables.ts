...
/**
 * Defines an abstract BaseModel which enforces implementation for Angular- and TypeScript specific
 * methods.
 * Should be used for all models which don't have a parent class.
 *
 */
export abstract class BaseModel {
    ...
    private static clearFormArray(formGroup: FormGroup, key: string): FormArray | any {
        if (formGroup) {
        ...
        function getDirtyState(form: FormGroup): Object {
            return Object.keys(form.controls).reduce<Object>((dirtyState, controlKey) => {
                ...
                if (control instanceof FormArray) {
                    return control.controls.filter( (c) => { var tmp = getDirtyState(<FormGroup>c); return Object.entries(tmp).length === 0 ? false : true; })
                                           .map(    (c) => { getDirtyState(<FormGroup>c) });
                }
    ...
    protected castTo(typeStr: string, value: any): any {
        if (!(value === null || value === undefined)) {
            ...
            if (typeStr === 'Date')
            {
                ...
                if ( (<string>value).match && (<string>value).match(pattern) ){
